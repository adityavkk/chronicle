...
`GOCACHE=/private/tmp/chronicle-p0-segments-gocache`. The first lint attempt
could not resolve `proxy.golang.org` inside the restricted sandbox; the
authorized rerun completed and reported `0 issues.` No shared cache was cleaned.

- `env -u REDIS_URL ... make test`: all race-enabled packages passed. Timed
  packages were root `9.615s`, auth `2.101s`, cmd/chronicle `1.907s`, cmd/dsui
  `3.301s`, checker `4.828s`, metrics `3.905s`, protocol `2.204s`, store
  `10.025s`, Redis `34.590s`, segments `12.492s`, and webhook `12.794s`.
- `cd loadgen && ... make test`: 11 timed packages passed; the slowest was
  `stats 3.886s`. Four command packages had no test files.
- `... make lint`: `0 issues.`
...
- Segment conformance: 7/7 cells and 2,324/2,324 assertions passed:
  ...
  `reads=0,seals=238`; every serving cell reported `reads=76,seals=238`.
- Direct Porcupine `store-linz`: 4/4 cells passed with one path partition:
  off 2,488 operations, Redis chunks 1,779, local files 1,606, object cache
  2,427.
- Toxiproxy Porcupine `store-linz`: 4/4 cells passed under randomized Redis
  partitions and latency with one path partition: off 2,213 operations, Redis
  chunks 1,732, local files 1,613, object cache 1,183.
- Verbose deterministic P0/P1 segment regressions: 10/10 top-level tests passed,
  including Redis and file/object publish-versus-GC, Redis and local
  delete/recreate, mixed-version seal, pinned snapshot, TTL-neutral seal,
  client TTL touch, shadow verification/repair, and Redis repair. The command
  reported `ok .../store/segments 1.484s`.
- Verbose Redis regressions: incarnation persistence/recreate and
  `internal_atomic_read_does_not_renew_sliding_TTL` both passed; the command
  reported `ok .../store/redis 0.636s`.
- After excluding the unrelated pull-wake helper, the race-enabled checker
  package rerun reported `ok .../jepsen/checker 2.587s`.
