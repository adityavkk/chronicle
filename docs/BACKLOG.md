...
| 10 | [#117](https://github.com/adityavkk/chronicle/issues/117) | Adopt gh-aw for the agent CI workflows: spike a gh-aw source `.md` + compiled `.lock.yml` replacing `backlog-slot.yml`, preferring an open engine (opencode/pi) with Claude Code as fallback; keep the deterministic `backlog-check.sh` gate | M | Important hardening of the agent-automation security model (read-only agent, AWF firewall, typed safe-outputs), but nothing user-facing blocks on it and gh-aw is pre-1.0 — so it sits below the correctness, CI-protection, and cutover work | A committed gh-aw source + `.lock.yml` replaces `backlog-slot.yml`; on the next issue opened the agent runs read-only under AWF with an open engine and the slot-proposal PR passes `backlog-check.sh` |
