...
| P0.4 | **Confirm [`model_shard.go`](https://github.com/adityavkk/chronicle/blob/main/jepsen/checker/model_shard.go) wiring.** It still carries its SCAFFOLD header even though [`claim_shard.lua`](https://github.com/adityavkk/chronicle/blob/main/webhook/scripts/claim_shard.lua) and [`check_owner.lua`](https://github.com/adityavkk/chronicle/blob/main/webhook/scripts/check_owner.lua) have shipped. Verify the live driver in [`scenario_shard.go`](https://github.com/adityavkk/chronicle/blob/main/jepsen/checker/scenario_shard.go) actually exercises real Lua; if red, wire it. Until this is green, `INV-OWNER-01/02` are specified but unvalidated. | `property-based-testing` |
