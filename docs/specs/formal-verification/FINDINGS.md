...
**Where it bites.** Redis stores stream frames as ZSET members keyed by the offset string, and `read.lua` scans bounded pages with `ZRANGEBYLEX` over those string members ([`store/redis/scripts/read.lua:56`](https://github.com/adityavkk/chronicle/blob/main/store/redis/scripts/read.lua)). The read path's correctness rests entirely on **lexicographic member order equalling numeric frame order**. Past a 10^16-byte `ByteOffset` (~10 PB on one stream) the ZSET ordering inverts at the boundary and page reads return frames out of order or omit them. This is a silent data-plane corruption, not a crash. `Offset.Compare` ([`store/offset.go:128`](https://github.com/adityavkk/chronicle/blob/main/store/offset.go)) is correct (it compares the numeric fields), so the Go core and the Redis lex order *disagree* exactly in this region; the MemoryStore oracle would never reproduce it because it never uses the string order for reads.
...
- Redis reads bounded pages by lex-ranging over the **full offset string**, which encodes `ReadSeq` as the high-order 16 digits. `read.lua` passes the lower and snapshot-tail upper bounds to `ZRANGEBYLEX` with `LIMIT`; the lower bound is `"(<offset>\xff"` ([`store/redis/scripts/read.lua:56`](https://github.com/adityavkk/chronicle/blob/main/store/redis/scripts/read.lua)).
...
| **Data-plane reader liveness** | Typed `WaitForPage` long-polls in [`store/redis/notify.go`](https://github.com/adityavkk/chronicle/blob/main/store/redis/notify.go), plus the shared notification-backed SSE hub in [`sse_hub.go`](https://github.com/adityavkk/chronicle/blob/main/sse_hub.go); both use a 1 s defensive poll | **All** liveness work targets the `__ds` webhook control plane (INV-WAKE-01, INV-JEP-L1..L5). The HTTP reader's own wake dependency is never modelled. A lost pub/sub notification delays delivery until the defensive poll, which is directly user-visible latency. | TLA+ liveness under weak fairness, or a notification-loss integration race test. |
