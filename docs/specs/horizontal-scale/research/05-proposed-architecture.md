...
ds:{__ds:h}:ownership:slot:<h>        HASH  owner_id, owner_epoch, lease_expiry_ns
```
...
4. A would-be owner claims `ds:{__ds:h}:ownership:slot:<h>` with `claim_shard.lua`. The script
...
- **Ownership keys split by role.** `ds:{ownership}:members` remains cross-slot
  membership metadata; `slotKey(h)` is co-homed under `{__ds:h}` so owner-fenced
  schedule writes stay single-slot.
...
under the slot's `{__ds:h}` tag, and a package var in `scripts.go`
...
-- carries a stale epoch and is fenced. The ownership-layer analogue of
...
`ds:{__ds:h}:ownership:slot:<h>` (just at shard rather than slot granularity), claimed by the same
