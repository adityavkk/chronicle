...
| **T3** | Ownership exclusivity | Exactly one current owner per slot/shard; a deposed owner's schedule writes are `FENCED` (`check_owner.lua`) | slot-owner churn + GC-pause sim + partition (toxiproxy/iptables) + clock skew on the membership TTL | **porcupine** CAS-register model of the co-located owner slot record (`ds:{__ds}:owner:slot:<h>` in S=1, `ds:{__ds:h}:owner:slot:<h>` after slot homing) = `{owner_id, owner_epoch, lease_expiry_ns}`. **Acceptance gate** for the proposed `claim_shard.lua` — proves it's a real CAS, not a silently-dropping LWW |
...
- **Acceptance gates for the proposed mechanisms** (`claim_shard.lua`, `check_owner.lua`,
  `ds:{ownership}:members`, co-located owner slot records, the S-slot `{__ds:h}` tagging, the per-sub due-set): **T3, T5** and
