...
| **T3** | Ownership exclusivity | Exactly one current owner per slot/shard; a deposed owner's schedule writes are `FENCED` (`check_owner.lua`) | slot-owner churn + GC-pause sim + partition (toxiproxy/iptables) + clock skew on the membership TTL | **porcupine** CAS-register model of `ds:{__ds:h}:ownership:slot:<h> = {owner_id, owner_epoch, lease_expiry_ns}`. **Acceptance gate** for the proposed `claim_shard.lua` — proves it's a real CAS, not a silently-dropping LWW |
