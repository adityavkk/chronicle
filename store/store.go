// Package store defines the durable-stream storage contract.
//
// Ported from the Durable Streams reference Caddy plugin
// (packages/caddy-plugin/store @ 82f9963) so chronicle's Redis backend
// satisfies the identical interface and can evolve with upstream.
package store
