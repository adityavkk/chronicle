...
func run() int {
	...
	// 5. Scrape before snapshot.
	names := []string{"chronicle_fanout_seconds", "chronicle_fanout_subs"}
	...
	// 8. Compute deltas.
	total := after["chronicle_fanout_seconds{stage=total}"].sub(before["chronicle_fanout_seconds{stage=total}"])
	probe := after["chronicle_fanout_seconds{stage=probe}"].sub(before["chronicle_fanout_seconds{stage=probe}"])
	...
	rep := Report{
		...
		ProbeMeanMs:    probe.mean() * 1000,
		ProbeP99Ms:     probe.quantile(0.99) * 1000,
	...
	fmt.Fprintf(os.Stderr,
		"\nS=%d %s | rate=%.1f/s | fan-out over %.0fs: mean=%.2fms p50=%.2fms p99=%.2fms p999=%.2fms | probe p99=%.2fms | %.0f fan-outs | appends=%d errs=%d\n",
		rep.Subs, rep.Dispatch, rate, rep.WindowSeconds,
		rep.FanoutMeanMs, rep.FanoutP50Ms, rep.FanoutP99Ms, rep.FanoutP999Ms,
		rep.ProbeP99Ms, rep.FanoutTicks, rep.Appends, rep.AppendErrors)
...
type Report struct {
	...
	ProbeMeanMs    float64 `json:"probe_mean_ms"`
