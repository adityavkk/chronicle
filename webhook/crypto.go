...
type writeTokenPayload struct {
	Typ         string   `json:"typ"`
	Sub         string   `json:"sub"`
	Incarnation string   `json:"inc,omitempty"`
	Generation  int64    `json:"gen"`
...
func GenerateClaimWriteToken(tokenKey []byte, subID, incarnation string, generation int64, wakeID, holder string, shard int, streams []auth.StreamPath, now time.Time, ttl time.Duration, rand io.Reader) (string, error) {
	return generateWriteToken(tokenKey, subID, incarnation, generation, wakeID, holder, shard, streams, now, ttl, rand)
}

func generateWriteToken(tokenKey []byte, subID, incarnation string, generation int64, wakeID, holder string, shard int, streams []auth.StreamPath, now time.Time, ttl time.Duration, rand io.Reader) (string, error) {
	jti := make([]byte, 8)
	if _, err := io.ReadFull(rand, jti); err != nil {
