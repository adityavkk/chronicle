...
func TestInlineFence_ArmWake(t *testing.T) {
	...
	res, err = s.ArmWakeOwned(stale, "s1", time.Now(), 1000, true, "wk-2")
	if err != nil {
		t.Fatal(err)
	}
	if !res.Fenced {
...
func TestInlineFence_AckAboveGenFence(t *testing.T) {
	...
	st, err := s.AckOwned(live, "s1", arm.Generation, arm.WakeID, arm.Generation, false, nil, now, 60000)
	if err != nil {
		t.Fatal(err)
	}
	if st != "OK" {
	...
	st, err = s.AckOwned(stale, "s1", arm.Generation, arm.WakeID, arm.Generation, false, nil, now, 60000)
	if err != nil {
		t.Fatal(err)
	}
	if st != "FENCED" {
...
func TestInlineFence_ReleaseAboveGenFence(t *testing.T) {
	...
	if st, err := s.ReleaseOwned(live, "s1", arm.Generation, arm.WakeID, arm.Generation); err != nil || st != "OK" {
	...
	arm, _ = s.ArmWakeUnscoped("s1", now, 60000, true, "wk-2")
	st, err := s.ReleaseOwned(stale, "s1", arm.Generation, arm.WakeID, arm.Generation)
	if err != nil {
		t.Fatal(err)
	}
	if st != "FENCED" {
...
func TestInlineFence_NoScopeSkipsCheck(t *testing.T) {
	...
	if st, err := s.AckUnscoped("s1", arm.Generation, arm.WakeID, arm.Generation, false, nil, now, 60000); err != nil || st != "OK" {
