...
printf '| S subs | fanout_p99_ms | p99_slots_probed | SLO | job_status |\n' >>"$RESULTS"
printf '|---|---|---|---|---|\n' >>"$RESULTS"
...
for S in 2 4 8 256; do
  ...
  LOGS=$(job_logs fanoutscale)
  P99=$(printf '%s' "$LOGS" | grep -o '"fanout_p99_ms":[0-9.]*' | head -1 | cut -d: -f2 || echo "N/A")
  # The epic exposes the occupied-slots bitmap effect as chronicle_fanout_slots_probed
  # (NOT the old probe-duration stage): it should stay a small constant even at S=256.
  SLOTS=$(printf '%s' "$LOGS" | grep -o '"p99_slots_probed":[0-9.]*' | head -1 | cut -d: -f2 || echo "N/A")
  STATUS=$(job_passed fanoutscale && echo "PASS" || echo "FAIL")

  printf '| %d | %s | %s | <%dms | %s |\n' "$S" "${P99}" "${SLOTS}" "$SLO" "${STATUS}" >>"$RESULTS"
  info "S=${S}: fanout_p99=${P99}ms p99_slots_probed=${SLOTS} → ${STATUS}"
  rm -rf "$P3_TMP"
done
