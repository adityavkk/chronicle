...
- `claim_shard.lua` CAS over `ds:{__ds:h}:ownership:slot:<h>` + `check_owner.lua` (external POST only); membership heartbeat + HRW (FNV-1a + splitmix64 finalize, ~1/N reassignment) + slot-reconcile loop; workers gated by `ownedSlots() = HRW ∩ held-leases`; `sweepOnce` stays the UNGUARDED backstop. TOCTOU owner-epoch check inlined in arm_wake/ack/expire_lease/schedule_retry/release (gated on a non-empty epoch — the load-balanced external path is unchanged; the owner scope is threaded through the retry path too). New-owner CAS fires #13's `reconcile(scopeNewOwnerCAS)`. ReplicaID + 4 TTLs with invariant tests. Pure FCIS core; distinct `SlotID`/`OwnerEpoch`/`ReplicaID` types (no collision with #11's `ClaimShard`/`ShardKey`).
